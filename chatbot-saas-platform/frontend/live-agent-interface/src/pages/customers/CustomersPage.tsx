...
const CustomersPage = () => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return <Badge variant={variants[status] || "secondary"}>{t(`customers.status.${status}`)}</Badge>;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex items-center text-xs text-muted-foreground">
              ...
              {t("customers.changeFromLastMonth")}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t("customers.avgSatisfaction")}</CardTitle>
            <Star className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {customers?.length ? (customers.reduce((acc, c) => acc + (c.satisfactionRating || 0), 0) / customers.length).toFixed(1) : "0.0"}
            </div>
            <div className="flex items-center text-xs text-muted-foreground">
              <TrendingUp className="mr-1 h-3 w-3" />
              {t("customers.satisfactionIncrease")}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t("customers.avgResponseTime")}</CardTitle>
            <Clock className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{t("customers.avgResponseTimeValue")}</div>
            <div className="flex items-center text-xs text-muted-foreground">
              <TrendingUp className="mr-1 h-3 w-3" />
              {t("customers.responseTimeImprovement")}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t("customers.customerListDesc")}</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <TableRow key={customer.id}>
                  <TableCell>
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <div className="font-medium">{customer.name || t("customers.anonymous")}</div>
                  ...
                  <TableCell>
                    <div className="flex items-center">
                      ...
                      {customer.satisfactionRating?.toFixed(1) || t("customers.notAvailable")}
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center">
                      ...
                      {customer.location || t("customers.unknown")}
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center space-x-2">
          ...
          {filteredCustomers.length === 0 && !isLoading && (
            <div className="text-center py-8">
              ...
              <h3 className="mt-2 text-sm font-semibold">{t("customers.noCustomersFound")}</h3>
              <p className="mt-1 text-sm text-muted-foreground">{t("customers.noCustomersFoundDesc")}</p>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={isDetailsDialogOpen} onOpenChange={setIsDetailsDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>{t("customers.customerDetailsDesc")}</DialogDescription>
          </DialogHeader>
          {selectedCustomer && (
            <div className="space-y-6">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold">{selectedCustomer.name || t("customers.anonymous")}</h3>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <div className="flex items-center">
                        ...
                        <span className="text-sm font-medium">{selectedCustomer.satisfactionRating?.toFixed(1) || t("customers.notAvailable")}</span>
                      </div>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm">{t("customers.memberSince")}</span>
                      <span className="text-sm font-medium">{new Date(selectedCustomer.createdAt).toLocaleDateString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm">{t("customers.lastContact")}</span>
                      <span className="text-sm font-medium">
                        {selectedCustomer.lastContact ? new Date(selectedCustomer.lastContact).toLocaleDateString() : t("customers.never")}
                      </span>
                    </div>
                  </div>
                </div>
