...
import { fetchConversations, fetchRealtimeConversations } from '../../store/slices/conversationSlice';
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchRealtimeConversations());
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {conversations?.filter(c => c.status === 'active').length || 3}
            </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {t('dashboard.todayConversations')}
            </CardTitle>
            <BarChart3 className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {agentStats?.todayConversations || 24}
            </div>
            <div className="flex items-center text-xs text-muted-foreground">
              <TrendingUp className="mr-1 h-3 w-3" />
              +15% from yesterday
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {agentStats?.averageResponseTime || 2.3}m
            </div>
            <div className="flex items-center text-xs text-muted-foreground">
              <TrendingUp className="mr-1 h-3 w-3" />
              -12% from last week
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {t('dashboard.satisfactionRating')}
            </CardTitle>
            <Star className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {agentStats?.averageRating || 4.8}
            </div>
            <div className="flex items-center text-xs text-muted-foreground">
              <TrendingUp className="mr-1 h-3 w-3" />
              +0.3 from last week
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card className="col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              {t('dashboard.performanceTrendsDesc')}
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t('dashboard.recentConversationsDesc')}
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-4">
            {currentAgent ? (
              <div className="space-y-4">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <Badge variant={currentAgent.status === 'online' ? 'default' : 'secondary'}>
                      {t(`agent.status.${currentAgent.status}`)}
                    </Badge>
                  </div>
                </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Unable to load agent profile information.
                </p>
              </div>
            )}
          </CardContent>
