...
import { LayoutDashboard, Bot, Users, BarChart3, CreditCard, Settings, ChevronLeft, ChevronRight, Workflow } from "lucide-react";
...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onToggle }) => {
  ...
  return (
    <div className={`fixed inset-y-0 ${isRTL ? "right-0" : "left-0"} z-50 flex`}>
      <div
        className={`relative flex w-64 flex-col bg-white dark:bg-gray-800 shadow-lg transition-all duration-300 ${
          isOpen ? "translate-x-0" : isRTL ? "translate-x-48" : "-translate-x-48"
        }`}
      >
        <div className="flex h-16 items-center justify-between px-4 border-b border-gray-200 dark:border-gray-700">
          {branding.logo ? (
            <img src={branding.logo} alt={branding.companyName || "Logo"} className="h-8 w-auto" />
          ) : (
            <div className="text-xl font-bold text-gray-900 dark:text-white">{branding.companyName || "Tenant Dashboard"}</div>
          )}

          <button onClick={onToggle} className="p-1 rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
            {isRTL ? (
              isOpen ? (
