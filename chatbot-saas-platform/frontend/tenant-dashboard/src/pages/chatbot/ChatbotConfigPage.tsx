...
import { Bot, Upload, Save, RefreshCw, MessageSquare, Brain, Settings, FileText, Zap, Globe, Send, Plus, Trash2 } from "lucide-react";
...
const ChatbotConfigPage = () => {
  ...
  const [predefinedIntents, setPredefinedIntents] = useState<
    Array<{
      id: string;
      label: string;
      message: string;
  ...
  const handleToggleIntent = (intentId: string) => {
    setPredefinedIntents(predefinedIntents.map((intent) => (intent.id === intentId ? { ...intent, isActive: !intent.isActive } : intent)));
  };
  ...
  const handleFileDrop = async (files: FileList) => {
    ...
    console.log(`Starting upload of ${files.length} files via drag and drop`);
    setUploadError(null);
    setUploadSuccess(null);
    setIsUploading(true);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">{isCreateMode ? t("chatbot.createTitle") || "Create Chatbot" : t("chatbot.title")}</h1>
          <p className="text-muted-foreground">
            {isCreateMode ? t("chatbot.createSubtitle") || "Set up your first AI chatbot configuration" : t("chatbot.subtitle")}
          </p>
        </div>
            ...
            <Dialog open={isTestModalOpen} onOpenChange={setIsTestModalOpen}>
              ...
              <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
                <DialogHeader>
                  ...
                  <DialogDescription>Test your chatbot configuration with live messages. This uses your current form settings.</DialogDescription>
                </DialogHeader>
        ...
        <TabsContent value="personality" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("chatbot.botPersonalityDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="name">{t("chatbot.botName")}</Label>
                  <Input id="name" {...register("name", { required: t("validation.required") })} placeholder={t("chatbot.botNamePlaceholder")} />
              ...
              <div className="space-y-2">
                ...
                <Textarea id="personality" {...register("personality")} placeholder={t("chatbot.personalityPlaceholder")} rows={4} />
              </div>
        ...
        <TabsContent value="knowledge" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{t("chatbot.totalDocuments")}</CardTitle>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{t("chatbot.totalWords")}</CardTitle>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{t("chatbot.lastUpdated")}</CardTitle>
                <Globe className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-sm">
                  {knowledgeBaseStats.lastUpdated ? new Date(knowledgeBaseStats.lastUpdated).toLocaleDateString() : t("common.never")}
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("chatbot.uploadTrainingDataDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div
                className={`border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition-colors duration-200 ${
                  isDragOver ? "border-primary bg-primary/5 border-solid" : "border-muted-foreground/25 hover:border-primary/50 hover:bg-primary/5"
                } ${isUploading ? "opacity-50 cursor-not-allowed" : ""}`}
                onDragOver={handleDragOver}
                onDragEnter={handleDragEnter}
                onDragLeave={handleDragLeave}
                onDrop={handleDrop}
                onClick={!isUploading ? handleDivClick : undefined}
              >
                <Upload className={`mx-auto h-12 w-12 transition-colors ${isDragOver ? "text-primary" : "text-muted-foreground"}`} />
                <div className="mt-4">
                  <Label htmlFor="file-upload" className="cursor-pointer">
                    <span className="text-sm font-medium text-primary hover:text-primary/80">
                      {isUploading ? "Uploading..." : isDragOver ? "Drop files here" : t("chatbot.clickToUpload")}
                    </span>
                    {!isDragOver && <span className="text-sm text-muted-foreground"> {t("chatbot.orDragAndDrop")}</span>}
                  </Label>
                  <Input
                    id="file-upload"
                    type="file"
                ...
                <p className="text-xs text-muted-foreground mt-2">{t("chatbot.supportedFormats")}</p>
              </div>

              {uploadError && <div className="p-3 text-sm text-red-600 bg-red-50 border border-red-200 rounded-md">{uploadError}</div>}

              {uploadSuccess && <div className="p-3 text-sm text-green-600 bg-green-50 border border-green-200 rounded-md">{uploadSuccess}</div>}

              {isUploading && <div className="p-3 text-sm text-blue-600 bg-blue-50 border border-blue-200 rounded-md">Uploading files...</div>}
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="behavior" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("chatbot.behaviorSettingsDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label>{t("chatbot.enableEmojis")}</Label>
                  <p className="text-sm text-muted-foreground">{t("chatbot.enableEmojisDesc")}</p>
                </div>
                <Switch checked={watch("enableEmojis")} onCheckedChange={(checked) => setValue("enableEmojis", checked)} />
              </div>

              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label>{t("chatbot.enableTypingIndicator")}</Label>
                  <p className="text-sm text-muted-foreground">{t("chatbot.enableTypingIndicatorDesc")}</p>
                </div>
                <Switch checked={watch("enableTypingIndicator")} onCheckedChange={(checked) => setValue("enableTypingIndicator", checked)} />
              </div>

              <div className="space-y-2">
                <Label htmlFor="maxConversationLength">{t("chatbot.maxConversationLength")}</Label>
                <Input id="maxConversationLength" type="number" {...register("maxConversationLength", { min: 1, max: 100 })} />
                <p className="text-sm text-muted-foreground">{t("chatbot.maxConversationLengthDesc")}</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="responses" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("chatbot.customResponsesDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="welcomeMessage">{t("chatbot.welcomeMessage")}</Label>
                <Textarea id="welcomeMessage" {...register("welcomeMessage")} placeholder={t("chatbot.welcomeMessagePlaceholder")} rows={3} />
              </div>

              <div className="space-y-2">
                <Label htmlFor="fallbackMessage">{t("chatbot.fallbackMessage")}</Label>
                <Textarea id="fallbackMessage" {...register("fallbackMessage")} placeholder={t("chatbot.fallbackMessagePlaceholder")} rows={3} />
                <p className="text-sm text-muted-foreground">{t("chatbot.fallbackMessageDesc")}</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="intents" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Create quick-start buttons that appear when users first open the chat widget. These help users get started with common questions or
                requests.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {predefinedIntents.map((intent) => (
                      <div key={intent.id} className="flex items-center justify-between p-3 border rounded-lg">
                        ...
                        <div className="flex items-center gap-2">
                          <Switch checked={intent.isActive} onCheckedChange={() => handleToggleIntent(intent.id)} />
                          <Button variant="ghost" size="sm" onClick={() => handleRemoveIntent(intent.id)}>
